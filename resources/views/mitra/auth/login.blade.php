    ...
    <form method="POST" action="{{ route('mitra.login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                ...
                <span class="ms-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Ingat ID saya') }}</span>
            </label>
        </div>

        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
            <a class="underline text-lg text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('password.request') }}">
                {{ __('Lupa password?') }}
            </a>
            @endif
